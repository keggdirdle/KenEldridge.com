...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
 ...
 // $templateCache.put('app/partials/slideshow.html',
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
   ...
   */
  //this.templateUrl = 'app/partials/slideshow.html';
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
    function ($document, $injector, $uibModal, $timeout, ImageLoader) {
      ...
      Lightbox.getEventTitle = this.getEventTitle;
      Lightbox.getLat = this.getLat;
      Lightbox.getLng = this.getLng;
      Lightbox.getEventStateDate = this.getEventStateDate;
      Lightbox.getStartTime = this.getStartTime;
      Lightbox.getEndTime = this.getEndTime;
      ...
      Lightbox.setImage = function (newIndex) {
        ...
        var success = function (properties) {
          ...
          Lightbox.eventTitle = Lightbox.getEventTitle(image);
          Lightbox.lat = Lightbox.getLat(image);
          Lightbox.lng = Lightbox.getLng(image);
          Lightbox.eventStateDate = Lightbox.getEventStateDate(image);
          Lightbox.startTime = Lightbox.getStartTime(image);
          Lightbox.endTime = Lightbox.getEndTime(image);
          ...
          if(document.getElementById("map")) {
            initMap(Lightbox.lat, Lightbox.lng)
          }
        };
...
angular.module('bootstrapLightbox').directive('lightboxSrc', ['$window',
  'ImageLoader', 'Lightbox', function ($window, ImageLoader, Lightbox) {
    ...
    var calculateImageDisplayDimensions = function (dimensions, fullScreenMode) {
      ...
      var h = dimensions.height - 50;  //added the nav div
